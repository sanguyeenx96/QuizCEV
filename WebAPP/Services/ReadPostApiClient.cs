...
{
    public class ReadPostApiClient : IReadPostApiClient
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private readonly IConfiguration _configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public ReadPostApiClient(IHttpClientFactory httpClientFactory, IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<ApiResult<bool>> Create(ReadPostCreateRequest request)
        {
            ...
            var response = await client.PostAsync("/api/readpost/", httpContent);
        ...
        public async Task<ApiResult<bool>> Delete(int id)
        {
            ...
            var response = await client.DeleteAsync($"/api/readpost/{id}");
            var result = await response.Content.ReadAsStringAsync();
            if (response.IsSuccessStatusCode)
            {
                return JsonConvert.DeserializeObject<ApiSuccessResult<bool>>(result);
        ...
        public async Task<ApiResult<List<ReadPostVm>>> GetAll()
        {
            ...
            var response = await client.GetAsync("/api/readpost/");
            var result = await response.Content.ReadAsStringAsync();
            if (response.IsSuccessStatusCode)
            {
        ...
        public async Task<ApiResult<List<ReadPostVm>>> GetAllByCategory(int id)
        {
            ...
            var response = await client.GetAsync($"/api/readpost/GetAllByCategory/{id}");
            var result = await response.Content.ReadAsStringAsync();
            if (response.IsSuccessStatusCode)
            {
                return JsonConvert.DeserializeObject<ApiSuccessResult<List<ReadPostVm>>>(result);
        ...
        public async Task<ApiResult<ReadPostVm>> GetById(int id)
        {
            ...
            var response = await client.GetAsync($"/api/readpost/GetById/{id}");
            var result = await response.Content.ReadAsStringAsync();
            if (response.IsSuccessStatusCode)
            {
                return JsonConvert.DeserializeObject<ApiSuccessResult<ReadPostVm>>(result);
            }
            return JsonConvert.DeserializeObject<ApiErrorResult<ReadPostVm>>(result);
        ...
        public async Task<ApiResult<bool>> UpdateThumbImage(int id, ReadPostThumbImageUpdate request)
        {
            ...
            var json = JsonConvert.SerializeObject(request);
            var httpContent = new StringContent(json, Encoding.UTF8, "application/json");
            var response = await client.PatchAsync($"/api/readpost/updateThumbImage/{id}", httpContent);
            var result = await response.Content.ReadAsStringAsync();
            if (response.IsSuccessStatusCode)
            {
                return JsonConvert.DeserializeObject<ApiSuccessResult<bool>>(result);
