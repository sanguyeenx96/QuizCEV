...
{
    public class DiemChuYApiClient : IDiemChuYApiClient
    {
        private readonly IHttpClientFactory _httpClientFactory;
        private readonly IConfiguration _configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public DiemChuYApiClient(IHttpClientFactory httpClientFactory, IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<ApiResult<bool>> Create(Guid cauhoitrinhtuthaotacId, DiemChuYCreateRequest request)
        {
            ...
            var response = await client.PostAsync($"/api/diemchuy/create/{cauhoitrinhtuthaotacId}", httpContent);
        ...
        public async Task<ApiResult<bool>> Delete(int id)
        {
            ...
            var response = await client.DeleteAsync($"/api/diemchuy/Delete/{id}");
            var result = await response.Content.ReadAsStringAsync();
            if (response.IsSuccessStatusCode)
            {
                return JsonConvert.DeserializeObject<ApiSuccessResult<bool>>(result);
        ...
        public async Task<ApiResult<List<DiemChuYVm>>> GetAllByCauHoiTrinhTuThaoTacId(Guid id)
        {
            ...
            var response = await client.GetAsync($"/api/diemchuy/GetAllByCauHoiTrinhTuThaoTacId/{id}");
            var result = await response.Content.ReadAsStringAsync();
            if (response.IsSuccessStatusCode)
            {
        ...
        public async Task<ApiResult<bool>> Update(int id, DiemChuYUpdateRequest request)
        {
            ...
            var json = JsonConvert.SerializeObject(request);
            var httpContent = new StringContent(json, Encoding.UTF8, "application/json");
            var response = await client.PatchAsync($"/api/diemchuy/update/{id}", httpContent);
            var result = await response.Content.ReadAsStringAsync();
            if (response.IsSuccessStatusCode)
            {
                return JsonConvert.DeserializeObject<ApiSuccessResult<bool>>(result);
